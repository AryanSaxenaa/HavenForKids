...
const MISTRAL_EMBEDDING_DIMENSION = 1024;

export const EMBEDDING_DIMENSION: number = MISTRAL_EMBEDDING_DIMENSION;
...
export function getLLMConfig(): LLMConfig {
  ...
  if (provider ? provider === 'mistral' : process.env.MISTRAL_API_KEY) {
    if (EMBEDDING_DIMENSION !== MISTRAL_EMBEDDING_DIMENSION) {
      throw new Error('EMBEDDING_DIMENSION must be 1024 for Mistral');
    }
    return {
      provider: 'custom',
  ...
  if (process.env.LLM_API_URL) {
    ...
    const embeddingModel = process.env.LLM_EMBEDDING_MODEL ?? 'mistral-embed';
    return {
      provider: 'custom',
      url,
      chatModel,
