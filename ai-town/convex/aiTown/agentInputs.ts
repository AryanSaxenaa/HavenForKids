  ...
  agentFinishSendingMessage: inputHandler({
    ...
    handler: (game, now, args) => {
      ...
      if (args.leaveConversation) {
        // HAVEN safety: never let an agent end a conversation with a human child.
        // The guard in agent.ts should prevent this, but double-check here.
        const otherPlayerId = [...conversation.participants.keys()].find(
          (id) => id !== agent.playerId,
        );
        const otherPlayer = otherPlayerId ? game.world.players.get(otherPlayerId) : undefined;
        if (otherPlayer?.human) {
          console.log(
            `[HAVEN] Blocking agent ${agent.id} from leaving conversation with human player ${otherPlayer.id}`,
          );
        } else {
