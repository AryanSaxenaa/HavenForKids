...
export class Player {
  ...
  tick(game: Game, now: number) {
    if (this.human && this.lastInput < now - HUMAN_IDLE_TOO_LONG) {
      // HAVEN: Never kick a child who is in an active conversation.
      // They may be reading / thinking. Only idle-kick when truly idle.
      const inConversation = [...game.world.conversations.values()].some((c) =>
