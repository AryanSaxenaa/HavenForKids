...
export class Agent {
  ...
  tick(game: Game, now: number) {
    ...
    if (!conversation && !doingActivity && (!player.pathfinding || !recentlyAttemptedInvite)) {
      // HAVEN: Zero backend calls when offline. If no human is playing, agents do absolutely nothing.
      const humanInWorld = [...game.world.players.values()].some((p) => !!p.human);
      if (!humanInWorld) {
        return;
      }
    ...
    if (conversation && member) {
      ...
      if (member.status.kind === 'participating') {
        ...
        // HAVEN: Never force-leave a conversation with a human child.
        // Only apply duration/message limits to agent-agent conversations.
        const talkingToHuman = !!otherPlayer.human;
        const tooLongDeadline = started + MAX_CONVERSATION_DURATION;
        const messageLimitExceeded = conversation.numMessages > MAX_CONVERSATION_MESSAGES;
        if (!talkingToHuman && (tooLongDeadline < now || messageLimitExceeded)) {
