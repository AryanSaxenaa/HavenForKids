...
export const userStatus = query({
  ...
  handler: async (ctx, args) => {
    return args.clientToken || DEFAULT_NAME;
  },
});
...
export const joinWorld = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const tokenIdentifier = args.clientToken || DEFAULT_NAME;

    // Deterministic character assignment based on the name
    let hash = 0;
    ...
    return await insertInput(ctx, world._id, 'join', {
      name,
      character: characters[charIndex].name,
      description: `${name} is a human player in HAVEN`,
      tokenIdentifier,
    });
  },
});
